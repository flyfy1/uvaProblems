...
int inCircle(Point p, Point c, double r){
    ...
    return (fabs(Euc - rSq) < EPS) ? CIR_BORDER : Euc < rSq ? CIR_INSIDE : CIR_OUTSIDE;
}
...
// Find the enclosing circle of the given arr of points
int findEnclosingCircle(Point arr[3]){
}

bool testAllIn(vector<Point> v, Point c, double r){
    for(int k=0;k<v.size();k++){
        if(inCircle(v[k],c,r) == CIR_OUTSIDE){
            return false;
        }
    }
    return true;
...
int main(){
    ...
    while(cin >> n, n){
        ...
        for(int i=0;i<n && !canWork;i++){
            for(int j = i+1;j<n && !canWork;j++){
                Point c;
                if(circle2PtsRad(pv[i],pv[j],r,&c) && testAllIn(pv,c,r)){
                    canWork = true;
                } else if(circle2PtsRad(pv[j],pv[i],r,&c) && testAllIn(pv,c,r)){
                    canWork = true;
                }
            }
        }
